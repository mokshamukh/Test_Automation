...
public class Premier_CustomerChangeName extends CommonLibrary {
	...
	public void clickContactMethodTab() throws Exception {
		boolean stepResult = false;
		try {
			clickOnElement("Contact Methods Page", "Contact Methods", contactDetailsTab);
			stepResult = true;
		...
		try {
			if (isElementPresent(contactDetailsTabTitle)){
				if(!phoneNumber.equals("")){
					clickAfterWaitForElementToBeClickable("Contact Methods Page", "Phone Number", phoneNumVal);
					enterText("Contact Methods Page", "Phone Number", phoneNumVal, phoneNumber);
				}
				if(!email.equals("")){
					clickAfterWaitForElementToBeClickable("Contact Methods Page", "Email", emailVal);
					enterText("Contact Methods Page", "Phone Number", emailVal, email);
				}
				if(!webAddress.equals("")){
					clickAfterWaitForElementToBeClickable("Contact Methods Page", "Web Address", webAddressVal);
					enterText("Contact Methods Page", "Phone Number", webAddressVal, webAddress);
				}
				stepResult = true;
			}
		} catch (Exception e) {
			...
			if (stepResult == true) {
				System.out.println("Pass");
				new HTMLReportHelper().HtmlReportBody("Contact Methods Details", "Change contact methods Successfully", "Passed", driver, "Y");
			} else {
				System.out.println("fail");
				new HTMLReportHelper().HtmlReportBody("Contact Methods Details", "Could not change contact methods", "Failed", driver, "Y");
			}
		}
	}
	...
	public void clickRelationshipsTab() throws Exception {
		boolean stepResult = false;
		try {
			clickOnElement("Relationships Page", "Relationships", relationshipsTab);
			stepResult = true;
		...
		try {
			if(isElementPresent(relationshipsTitle)){
				...
				if (!buildRelationship_SSN.equals("")) {
					clickOnElement("Relationship Page", "Add Name Image",addNameImage);
					switchToWindowWithTitleContaining("Add Name");
					driver.switchTo().frame("bottom");
					if (isElementPresent(SearchTitle)) {
						enterText("Search Page", "Enter SSN Field", BuildRelationshipSSN, buildRelationship_SSN);
						clickOnElement("Search Page", "Submit Button",SubmitButton_SearchScreen);
						waitForPresenceOfElement("Search Page", "Searched Name List", NameList);
						clickOnElement("Search Page", "Searched Result Link",SearchResult);
						switchToWindowWithTitleContaining("Institution 01 - REPUBLIC BANK UAT");
						driver.switchTo().frame("Main");
					}
					if (!build_Relationship.equals("")) {
						selectElementByVisibleText("Relationship Page", "Name Relationship field", RelationshipName, build_Relationship);
					}
				}
				stepResult = true;
			}
	...
	public void AddBeneficiary(String beneficialOwnerName,String beneficial_Relationship,String beneficial_Percent,String buildRelationship_2_Flag,String buildRelationship_2_SSN,String build_Relationship_2,String beneficialOwner_2_Flag,String beneficialOwnerName_2,String beneficial_Relationship_2,String beneficial_Percent_2) throws Exception {
		...
		try {
			...
			if (stepResult == true) {
				System.out.println("Pass");
				new HTMLReportHelper().HtmlReportBody("Add a c", "Create Build Relationship page Successfully", "Passed", driver, "Y");
			} else {
				System.out.println("fail");
	...
	public void saveDetails() throws Exception {
		boolean stepResult = false;
		try {
			clickOnElement("Relationships Page", "Save", saveButton);
